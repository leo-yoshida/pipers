#Setwd, all files should be in the same directory
#In this way, files are imported solely by their name
...
library("readr")
tx2gene_parb <- read_csv("cat_parb_tx2gene.txt")
tx2gene_pgau <- read_csv("cat_pgau_tx2gene.txt")
tx2gene_pmal <- read_csv("cat_pmal_tx2gene.txt")
tx2gene_ptub <- read_csv("cat_ptub_tx2gene.txt")
...
ddsTxi_parb <- ddsTxi_parb[rowSums(counts(ddsTxi_parb) >= 10) >= 3, ]
...
ddsTxi_pgau <- ddsTxi_pgau[rowSums(counts(ddsTxi_pgau) >= 10) >= 3, ]
...
ddsTxi_pmal <- ddsTxi_pmal[rowSums(counts(ddsTxi_pmal) >= 10) >= 3, ]
...
ddsTxi_ptub <- ddsTxi_ptub[rowSums(counts(ddsTxi_ptub) >= 10) >= 3, ]

#Isoforms level analysis, (comment all if gene-level analysis is desired)
...
rlog_dds_parb <- rlog(ddsTxi_parb)
rlog_dds_pgau <- rlog(ddsTxi_pgau)
rlog_dds_pmal <- rlog(ddsTxi_pmal)
rlog_dds_ptub <- rlog(ddsTxi_ptub)
...
#Resultados com logFC (lfcThreshold) customizados e limiar de p-valores (alpha) customizados 
...
write.table(parb_normalized_counts, file = "parb_normcounts.tsv", row.names=TRUE, col.names=NA, sep="\t")
write.table(pgau_normalized_counts, file = "pgau_normcounts.tsv", row.names=TRUE, col.names=NA, sep="\t")
write.table(pmal_normalized_counts, file = "pmal_normcounts.tsv", row.names=TRUE, col.names=NA, sep="\t")
write.table(ptub_normalized_counts, file = "ptub_normcounts.tsv", row.names=TRUE, col.names=NA, sep="\t")

#Tabela counts transformados rlog
